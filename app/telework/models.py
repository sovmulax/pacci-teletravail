...
class TypePersonnel(SafeDeleteModel, HistoricalRecords):
    ...
    id = models.UUIDField("ID", primary_key=True, default=uuid.uuid4,
                          editable=False)
    nom = models.CharField(max_length=50, choices=[('agent', 'Agent'), ('superieur_hierarchique', 'Supérieur hiérarchique')])
...
class Personnel(SafeDeleteModel, HistoricalRecords):
    ...
    photo = models.ImageField("Photo", upload_to='profile_photos/', blank=True, null=True)
    def __str__(self):
        return f"{self.user.username}  "
class MotifRefus(models.Model):
    demande = models.OneToOneField('DemandeTeleTravail', on_delete=models.CASCADE, related_name='motif_refus')
    motif = models.TextField()

    def clean(self):
        # Vérifier si la demande associée a le statut 'Refusé'
        if self.demande and self.demande.statut != 'en attente':
            raise ValidationError("La demande associée doit avoir le statut 'en attente' pour ajouter un motif de refus.")
    ...
    def get_refused_demands(cls):
        ...
        return DemandeTeleTravail.objects.filter(statut='Refusé').exclude(id__in=motifs_refuses)
...
class DemandeTeleTravail(SafeDeleteModel, HistoricalRecords):
    ...
    MOTIFS_CHOICES = [
        ...
        ('Demandes_personnelles_exceptionnelles', 'Demandes personnelles exceptionnelles'),
        ('Autres', 'Autres'),
    ]
    ...
    autre_motif = models.CharField(max_length=50, blank=True)  # Rendre autre_motif facultatif
    date_debut = models.DateField()
    date_fin = models.DateField()
    lieu = models.CharField(max_length=100, choices=LIEU_CHOICES, default='Domicile')
    autre_champ_si_nécessaire = models.CharField(max_length=50, blank=True)  # Rendre autre_champ_si_nécessaire facultatif
    commentaire = models.TextField(blank=True)

    date_demande = models.DateTimeField(default=timezone.now, editable=False)
    statut = models.CharField(max_length=10, default='en attente', editable=False, choices=[
        ('en attente', 'En attente'),
        ('Refusé', 'Refusé'),
        ('Accepté', 'Accepté')
    ])
    date_signature_superieur = models.DateTimeField(default=timezone.now, editable=False)
    ...
    def accepter_demande(self, superieur):
        if superieur.type_personnel.nom == 'superieur_hierarchique':
            ...
            raise ValidationError("Seuls les supérieurs hiérarchiques peuvent accepter une demande de télétravail.")
    ...
    def refuser_demande(self, superieur):
        if superieur.type_personnel.nom == 'superieur_hierarchique':
            ...
            raise ValidationError("Seuls les supérieurs hiérarchiques peuvent refuser une demande de télétravail.")

    @receiver(pre_save, sender='app.DemandeTeleTravail')
    def supprimer_demandes_expirees(sender, instance, **kwargs):
        if instance.statut == 'en attente' and instance.date_fin < timezone.now().date():
            instance.delete()
    ...
    def clean(self):
        ...
        if self.date_debut and self.date_fin:
            ...
            if days_difference > 2:
                raise ValidationError("La période de télétravail ne peut pas dépasser deux jours.")

            if days_difference  <=0:
                raise ValidationError("La période de télétravail ne peut pas être negatif .")
        if self.motif != 'Autres':
            self.autre_motif = ''  # Réinitialise autre_motif si le motif n'est pas "Autres"
            self.autre_champ_si_nécessaire = ''  # Réinitialise autre_champ_si_nécessaire
...
class LiaisonAgentSuperieur(SafeDeleteModel, HistoricalRecords):
    ...
    superieur = models.ForeignKey(Personnel, related_name='superieur', on_delete=models.CASCADE)
