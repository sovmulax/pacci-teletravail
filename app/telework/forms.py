...
class DemandeTeleTravailForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(DemandeTeleTravailForm, self).__init__(*args, **kwargs)
        self.fields['motif'] = forms.ChoiceField(choices=self.MOTIFS_CHOICES, label='Motif',
    ...
    autre_motif = forms.CharField(label='Spécifiez le motif', required=False,
    ...
    lieu = forms.ChoiceField(choices=LIEU_CHOICES, label='Lieu', widget=forms.Select(attrs={'class': 'form-select'}))
    autre_champ_si_nécessaire = forms.CharField(label='Préciser le lieu', required=False,
                                                widget=forms.TextInput(attrs={'class': 'form-control'}))

    class Meta:
        model = DemandeTeleTravail
        fields = ['motif','autre_motif', 'date_debut', 'date_fin', 'lieu','autre_champ_si_nécessaire', 'commentaire']
        widgets = {
            'date_debut': DateTimeInput(attrs={"type": "date", 'class': 'form-control'}),
    ...
    def clean(self):
        ...
        if motif == 'Autres' and not autre_motif:
            raise forms.ValidationError("Veuillez spécifier le motif dans le champ 'Autres'.")
...
class MotifRefusForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['demande'].queryset = DemandeTeleTravail.objects.filter(statut='en attente')
        if current_demande:
